...
# --- [2. 템플릿 데이터베이스 - 원본 그대로 유지] ---
templates = {
    'MELV': {
        'commerce': {
            'subject': '[제안] 뷰티 브랜드 멜브(MELV) 커머스 협업 제안드립니다.',
            'body': """안녕하세요 {nickname}님, MELV MD 박혜란입니다.<br><br>저희 뷰티 브랜드 멜브(MELV)의 제품 무드와 시너지가 날 것 같아 함께 수익 쉐어형 커머스(공구)를 진행하고자 제안드립니다.<br><br><b>[브랜드 및 제품 소개]</b><br>브랜드명: MELV(멜브)<br>주력 제품: 립시럽, 립타투<br>성과 지표: 카카오톡 선물하기 뷰티 랭킹 1위 달성<br>제품 확인하기: <a href='https://a-bly.com/app/markets/108879/'>에이블리 링크</a><br><br>상세 제안서 검토를 희망하시거나 미팅이 가능하시다면 회신 부탁드립니다.<br><br>감사합니다.<br>박혜란 드림"""
        },
        'seeding': {
    ...
    'SOLV': {
        'commerce': {
            ...
            'body': """안녕하세요 {nickname}님, SOLV 담당자 박혜란입니다.<br><br>저희 브랜드 솔브(SOLV)와 함께 시너지를 낼 수 있는 수익 쉐어형 커머스 파트너십을 제안드립니다.<br><br><b>주력 제품:</b> 더블세럼 모델링팩<br>제품 확인하기: <a href='https://solv.co.kr/aboutus/productstory.html'>공식몰 링크</a><br><br>감사합니다.<br>박혜란 드림"""
        },
        'seeding': {
            'subject': '[협찬] 뷰티 브랜드 솔브(SOLV) 화잘먹 모델링팩 시딩 제안드립니다.',
            'body': """안녕하세요 {nickname}님, SOLV 담당자 박혜란입니다.<br><br>크리에이터님 특유의 맑은 분위기와 저희 브랜드 솔브(SOLV)가 만나면 좋은 시너지가 날 것 같아 제품 시딩을 제안드립니다.<br><br>감사합니다.<br>박혜란 드림"""
        }
    },
    'UPPR': {
        'commerce': {
            'subject': '[제안] 라이프/패션 브랜드 어퍼(UPPR) 커머스 협업 제안드립니다.',
            'body': """안녕하세요 {nickname}님, UPPR 담당자 박혜란입니다.<br><br>저희 브랜드 어퍼(UPPR)의 힙한 무드와 크리에이터님의 핏이 아주 잘 맞을 것 같아 협업을 제안드립니다.<br><br>감사합니다.<br>박혜란 드림"""
        },
        'seeding': {
            'subject': '[협찬] 라이프/패션 브랜드 어퍼(UPPR) 제품 시딩 제안드립니다.',
            'body': """안녕하세요 {nickname}님, UPPR 담당자 박혜란입니다.<br><br>데일리룩 코디를 잘 보고 있어 저희 어퍼 브랜드의 제품 시딩을 제안드립니다.<br><br>감사합니다.<br>박혜란 드림"""
        }
    }
}
...
with tab1:
    ...
    with c1: platform = st.radio("발송 계정 선택", ["Gmail", "Naver"], horizontal=True)
    with c2: b_choice = st.selectbox("브랜드 선택", ["MELV", "SOLV", "UPPR"])
    with c3: t_choice = st.selectbox("템플릿 종류", ["commerce", "seeding"])
    
    s_name = st.text_input("담당자 서명 이름", "박혜란")
    ...
    if n_list and e_list:
        ...
        s_temp = templates[b_choice][t_choice]
        p_body = s_temp['body'].format(nickname=n_list[0])
        
        with st.container(border=True):
            st.markdown(f"**제목:** {s_temp['subject']}")
            st.markdown(f"<div style='font-size: 14px;'>{p_body}</div>", unsafe_allow_html=True)
            ...
            try:
                ...
                for i, (nick, email) in enumerate(zip(n_list, e_list)):
                    ...
                    conn.execute("INSERT INTO send_log (brand, type, nickname, email, status, sent_at) VALUES (?, ?, ?, ?, ?, ?)",
                                 (b_choice, t_choice, nick, email, status, datetime.datetime.now().strftime("%y/%m/%d %H:%M")))
