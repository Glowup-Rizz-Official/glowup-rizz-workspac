...
templates = {
    'MELV': {
        ...
        'seeding': {
            ...
            'body': """안녕하세요 {nickname}님, MELV MD 박혜란입니다.<br><br>크리에이터님의 뷰티 콘텐츠 무드와 저희 브랜드 멜브(MELV)의 결이 정말 잘 어울릴 것 같아, 제품 시딩(협찬)을 제안드립니다.<br><br>이번 제품 시딩은 직접 사용해 보신 후 인스타그램에 1회 업로드해 주시는 형태로 진행됩니다. 평소 보여주시는 예쁜 감성 그대로 자연스럽게 담아주시면 됩니다! ☺️<br><br>제품 수령을 희망하신다면 [성함/연락처/주소]를 기재하여 편하게 회신 부탁드리겠습니다.<br><br>감사합니다.<br>박혜란 드림"""
        }
    },
    'SOLV': {
        'commerce': {
            'subject': '[제안] 뷰티 브랜드 솔브(SOLV) 커머스 협업 제안드립니다.',
            'body': """안녕하세요 {nickname}님, SOLV 담당자 박혜란입니다.<br><br>저희 브랜드 솔브(SOLV)와 크리에이터님이 함께 시너지를 낼 수 있는 수익 쉐어형 커머스 파트너십을 제안드립니다.<br><br><b>주력 제품:</b> 더블세럼 모델링팩 (3초 컷 홈에스테틱)<br>제품 확인하기: <a href='https://solv.co.kr/aboutus/productstory.html'>공식몰 링크</a><br><br>상세 제안서 검토나 미팅 일정을 잡고자 하신다면 편하게 회신 부탁드립니다.<br><br>감사합니다.<br>박혜란 드림"""
        },
        'seeding': {
            'subject': '[협찬] 뷰티 브랜드 솔브(SOLV) 화잘먹 모델링팩 시딩 제안드립니다.',
            'body': """안녕하세요 {nickname}님, SOLV 담당자 박혜란입니다.<br><br>크리에이터님 특유의 맑은 분위기와 저희 브랜드 솔브(SOLV)가 만나면 좋은 시너지가 날 것 같아 제품 시딩(협찬)을 제안드립니다.<br><br>평소 크리에이터님 특유의 무드로 '화잘먹 윤광 코팅' 효과를 일상 속에서 예쁘게 보여주시면 충분합니다. 🩵<br><br>제품 수령을 희망하신다면 [성함/연락처/주소]를 기재하여 편하게 회신 부탁드리겠습니다.<br><br>감사합니다.<br>박혜란 드림"""
        }
    },
    'UPPR': {
        'commerce': {
            'subject': '[제안] 라이프/패션 브랜드 어퍼(UPPR) 커머스 협업 제안드립니다.',
            'body': """안녕하세요 {nickname}님, UPPR 담당자 박혜란입니다.<br><br>저희 브랜드 어퍼(UPPR)의 힙한 무드와 크리에이터님의 핏이 아주 잘 맞을 것 같아, 판매 수익 쉐어형 커머스(공구) 진행을 제안드립니다.<br><br><b>주력 제품:</b> 코듀로이 볼캡 or 시그니처 체크셔츠<br>제품 확인하기: <a href='https://smartstore.naver.com/uppr'>스토어 링크</a><br><br>관련하여 상세 제안서 확인이 필요하시거나 논의할 부분이 있으시다면 언제든 회신 부탁드립니다.<br><br>감사합니다.<br>박혜란 드림"""
        },
        'seeding': {
            'subject': '[협찬] 라이프/패션 브랜드 어퍼(UPPR) 제품 시딩 제안드립니다.',
            'body': """안녕하세요 {nickname}님, UPPR 담당자 박혜란입니다.<br><br>평소 올려주시는 감각적인 데일리룩 코디를 잘 보고 있어, 저희 어퍼 브랜드의 제품 시딩(협찬)을 제안드립니다.<br><br>평소 즐겨 입으시는 데일리룩에 가볍게 툭 매치해서 힙하고 편안한 꾸안꾸룩으로 연출해 주시면 정말 좋을 것 같아요! 🧢<br><br>제품 수령을 원하신다면 [성함/연락처/주소]와 함께 [원하시는 품목 및 사이즈]를 기재하여 회신 부탁드리겠습니다.<br><br>감사합니다.<br>박혜란 드림"""
        }
    }
}
...
with tab1:
    ...
    with col_in1:
        st.session_state.nick_input = st.text_area("1. 닉네임 리스트 (줄바꿈 구분)", value=st.session_state.nick_input, height=150)
    with col_in2:
        st.session_state.email_input = st.text_area("2. 이메일 리스트 (줄바꿈 구분)", value=st.session_state.email_input, height=150)
    ...
    if nicks and emails:
        ...
        with st.container(border=True):
            ...
            if os.path.exists(IMAGE_PATH):
                st.image(IMAGE_PATH, width=220, caption="첨부 명함 (Small)")

        if st.button(f"🚀 {len(target_df)}명에게 {platform}로 발송 시작", type="primary", use_container_width=True):
            ...
            try:
                ...
                # 플랫폼별 서버 연결 분기
                if platform == "Naver":
                    server = smtplib.SMTP_SSL("smtp.naver.com", 465) # 네이버는 465 SSL
                else:
                    server = smtplib.SMTP("smtp.gmail.com", 587) # 지메일은 587 TLS
                    server.starttls()
                ...
                for i, row in target_df.iterrows():
                    ...
                    if img_data:
                        image = MIMEImage(img_data, _subtype=file_ext) [cite: 1]
