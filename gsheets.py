...
class GoogleSheetsManager:
    ...
    def get_all_existing_identifiers(self):
        """[핵심 추가] 기존 시트에 있는 모든 닉네임, 이메일, 프로필 링크를 긁어옵니다."""
        if not self.spreadsheet: return set()
        existing = set()
        for brand in ["MELV", "SOLV", "UPPR"]:
            try:
                records = self.spreadsheet.worksheet(brand).get_all_records()
                for r in records:
                    for key in ['인스타', '틱톡', '블로그', '이메일', '닉네임', '유튜브']:
    ...
    def append_searched_data(self, brand_name, df_selected):
        ...
        try:
            ...
            return True, f"{len(rows_to_insert)}명의 크리에이터가 '{brand_name}' 탭에 저장되었습니다!"
